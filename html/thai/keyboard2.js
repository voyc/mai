...
//	this.post = new voyc.Post('flash','http://flash.voyc.com','../flash/index.html');
	...
	this.shiftcase = 1;  // 1:lowercase, 2;uppercase
}
...
voyc.Keyboard.prototype = {
	...
	drawKeypad: function() {
		...
		sanal += '<tr><td><td class="hdr pool" tag="mid">Middle Class</td></tr>';
		sanal += '<tr><td><td class="hdr pool" tag="high">High Class</td></tr>';
		sanal += '<tr><td><td class="hdr pool" tag="low">Low Class</td></tr>';
		sanal += '<tr><td><td class="hdr pool" tag="sonorant">Sonorant</td></tr>';
		sanal += '<tr><td><td class="hdr pool" tag="sanskrit">Sanskrit</td></tr>';
...
//		s += '<tr class="colhdrrow"><td class="noborder"></td><td class="hdr" colspan="7">Consonant</td><td class="hdr" colspan="6">Vowel</td><td class="hdr"></td></tr>';
		...
		for (var row=1; row<=numrows; row++) {
			...
			for (var col=1; col<=(numcols); col++) {
				var id = 'cell_' + row + '_' + col + '_' + '2';
				s += '<td class="key" id="' +id+ '">&nbsp;</td>';
			}
			s += '</tr>';
			s += '<tr lc>';
			for (var col=1; col<=(numcols); col++) {
				var id = 'cell_' + row + '_' + col + '_' + '1';
				s += '<td class="key" id="' +id+ '">&nbsp;</td>';
			}
			s += '</tr>';
		}
	...
	attachDomEventHandlers: function() {
		...
		// shifter
		var shift1id = 'cell_4_1_1';	
		var shift2id = 'cell_4_1_2';
		voyc.$(shift1id).innerHTML = '<img src=thai/i/uparrowoutline.png height=28px>';
		voyc.$(shift1id).addEventListener('click', function(event) {
			self.shift(1);
		}, false);
		voyc.$(shift2id).innerHTML = '<img src=thai/i/uparrowfilled.png height=28px>';
		voyc.$(shift2id).addEventListener('click', function(event) {
			self.shift(2);
		}, false);
	...
	shift(shiftcase) {
		...
		var lcattr = 'table-row';
		var ucattr = 'none';
		if (this.shiftcase == 2) {
			ucattr = 'table-row';
			lcattr = 'none';
		}
		var list = document.querySelectorAll('tr[lc]');
		for (var i=0; i<list.length; i++) {
			list[i].style.display = lcattr;
		}
		var list = document.querySelectorAll('tr[uc]');
		for (var i=0; i<list.length; i++) {
			list[i].style.display = ucattr;
		}
	},
	...
	setLanguage: function(lang) {
		voyc.appendScript('thai/' + lang + '.js');
...
window['voyc']['onAlphabetLoaded'] = function(alphabet) {
	...
	voyc.keyboard.setup(voyc.$('keyboard'));
}
